...
function PalmTree(pt, CFrame, Segments, Modifier, StartSize, StartAngle, Leaves, LeafSegments, LeafSegmentModifier, LeafStartSize, LeafStartAngle, LeafStartVariation, ExtensionsPerLeaf, Colors, Materials)
	...
	cylinder(Start)
	local trunkSmooth = part(Palm, V3(StartSize.X, StartSize.X, StartSize.X), Start.CFrame*CN(0, Start.Size.Y/2, 0), Colors[1], Materials[1])
	mesh(trunkSmooth, V3(1, 1, 1))
	...
	for P = 1, Segments-1 do
		...
		local palmSmooth = part(Palm, V3(PalmSegment.Size.X, PalmSegment.Size.X, PalmSegment.Size.X), PalmSegment.CFrame*CN(0, PalmSegment.Size.Y/2, 0), Colors[1], Materials[1])
		mesh(palmSmooth, V3(1, 1, 1))
	...
	for l = 1, n do
		...
		for _, B in pairs(structure[l-1]) do
			...
			for s = 1, splits do
				...
				local startAngle = CA((math.sin(math.rad(newAngle))*spread+MR(0, random1)+MR(-random2, random2))/curveResolution, MR(-twist, twist)/curveResolution, (math.cos(math.rad(newAngle))*spread+MR(0, random1)+MR(-random2, random2))/curveResolution)
				...
				local curveAngle = CA((math.sin(math.rad(newAngle))*spread+MR(0, random1)+MR(-random2, random2))/curveResolution, MR(-twist, twist)/curveResolution, (math.cos(math.rad(newAngle))*spread+MR(0, random1)+MR(-random2, random2))/curveResolution)
				
				if l == n then
					--barkColor = "Really red"
					--newHeight = newHeight+4
					
					--Z, Y, X order for CFrame.Angles
				end
				...
				for c = 1, curveResolution-1 do
					...
					if --[[l == n]]false then
						...
						local t = lastBranch[2]*CN(0, 10, 0)
						local pointing = CN(lastBranch[2].p, V3(chosen[2].X, cf.Y, chosen[2].Z))*CA(-90, 0, 0)
						
						local test = lastBranch[2]:toObjectSpace(pointing)
						
						--part(newTree, V3(1, 1, 1), lastBranch[2]*test, "Royal purple", "SmoothPlastic", 0)
						
						local X, Y, Z = test:toEulerAnglesXYZ()
						local this = CA(0, 0, math.deg(Z/curveResolution))*CA(0, math.deg(Y/curveResolution), 0)*CA(math.deg(X/curveResolution), 0, 0)
						
						--part(newTree, V3(1, 1, 1), lastBranch[2], "Royal purple", "SmoothPlastic", 0)
						
						--part(newTree, V3(1, 1, 1), startTransform[2], "Royal purple", "SmoothPlastic", 0)
						--part(newTree, V3(1, 1, 1), chosen[2], "Really red", "SmoothPlastic", 0)
						
						--curveAngle = curveAngle:lerp(this, 0.5)
						curveAngle = this
...
weepingWillow(
map, --parent
CN(MR(-100, 100), -5, MR(-100, 100)), --cframe
V2(3, 3), --trunk size
6, --levels of recursion
{1/5, 1/6}, --scaling
8, --curve resolution
30, --spread
36, --spread modifier/randomness 1
31, --randomness 2
